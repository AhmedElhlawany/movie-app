document.querySelector("#Popular").addEventListener("click", getPopMovies(20))
document.querySelector("#search").addEventListener("input", search);


async function getPopMovies(numberOfMovies) {
    try {
        let response = await fetch('https://api.themoviedb.org/3/movie/popular?language=en-US&page=1', {
            headers: {
        ...
        let popularMovies = finalResponse.results.slice(0, numberOfMovies);
        console.log(popularMovies);
        window.popularmovies = popularMovies;
        displayPopMovies(popularMovies);
    } catch (error) {
        console.log(error);
    }
}
...
function displayPopMovies(movies) {
    let popcontainer = '';
    ...
    document.getElementById("popular-movie-card").innerHTML = popcontainer;
}


function search(event) {
    const searchValue = event.target.value.toLowerCase();
    const filteredMovies = window.popularmovies.filter(movie => movie.title.toLowerCase().includes(searchValue));
    displayPopMovies(filteredMovies);
}