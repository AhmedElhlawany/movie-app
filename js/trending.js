document.querySelector("#Trending").addEventListener("click", () => trendMovies(20));

document.querySelector("#search").addEventListener("input", search);

async function trendMovies(numberOfMovies) {
    try {
        ...
        let trendResponse = await response.json();
        
        let trendMovies = trendResponse.results.slice(0, numberOfMovies); 
        console.log(trendMovies);
        window.trendmovies = trendMovies;
        displayTrendMovies(trendMovies); 
    } catch (error) {
        console.log(error);
    }
}
...
function displayTrendMovies(movies) {
    ...
    document.getElementById("trend-movie-card").innerHTML = trendcontainer;
}

function search(event) {
    const searchValue = event.target.value.toLowerCase();
    const filteredMovies = window.trendmovies.filter(movie => movie.title && movie.title.toLowerCase().includes(searchValue));
    displayTrendMovies(filteredMovies);
}
