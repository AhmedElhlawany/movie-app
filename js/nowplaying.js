document.querySelector("#now-playing").addEventListener("click", () => nowPlayingMovies(20));

document.querySelector("#search").addEventListener("input", search);

async function nowPlayingMovies(numberOfMovies) {
    try {
        ...
        let nowResponse = await response.json();
        
        let nowPMovies = nowResponse.results.slice(0, numberOfMovies); 
        console.log(nowPMovies);
        
        
        window.nowmovies = nowPMovies;

        displayNowMovies(nowPMovies); 
    } catch (error) {
        console.log(error);
    }
}
nowPlayingMovies(20)
function displayNowMovies(movies) {
    let nowcontainer = '';
